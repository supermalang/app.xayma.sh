# /db-migration — Generate Supabase SQL Migration
...
## Purpose
Generate a SQL migration file for Supabase schema changes (new table, column, RLS policy, trigger).

## Output
Creates a timestamped migration file in `supabase/migrations/`:
```
supabase/migrations/20260326_121530_create_deployments_table.sql
```
...
create table xayma_app.deployments (
  ...
  name text not null,
  service_type text not null check (service_type in ('odoo', 'custom')),
